        ...
        public static void GenerateBindCode(UIMediumConfig config)
        {
            ...
            fileWriter.StartBlock();
            {
                foreach (var viewConfig in config.viewConfigList)
                {
                    ...
                    fileWriter.AppendLine("private {0} {1};", viewMonoScript.GetClass().Name, fieldName);
                }
                fileWriter.AppendLine();
                foreach (var viewConfig in config.viewConfigList)
                {
                    ...
                    fileWriter.AppendLine("public {0} {1}", viewMonoScript.GetClass().Name, publicFieldName);
                    fileWriter.StartBlock();
                    {
                        ...
                        fileWriter.StartBlock();
                        {
                            ...
                            fileWriter.StartBlock();
                            {
                                fileWriter.AppendLine("{0} = CreateController(\"{1}\") as {2};", privateFieldName, viewConfig.nameStr, viewMonoScript.GetClass().Name);
                            }
                            fileWriter.EndBlock();
                ...
                fileWriter.StartBlock();
                {
                    ...
                    fileWriter.StartBlock();
                    {
                        foreach (var viewConfig in config.viewConfigList)
                        {
                            ...
                            fileWriter.StartTab();
                            {
                                fileWriter.AppendLine("{0} = ctrl as {1};", privateFieldName, viewMonoScript.GetClass().Name);
            ...
            Debug.Log("[UIMediumConfigInspector] GenerateCode Finished: " + monoScript.GetClass().Name);
        }
    }
}
